(() => {
  ...
  function buyShopItem(item){
    ...
    else if(item==='rain-hat'&&!state.rainHat){state.rainHat=true;purchased=true;message='Sydvesten beskytter hoppekraften mot askeregn resten av turen.';}
    else if(item==='boots'&&!state.boots){state.boots=true;purchased=true;message='Støvlene beskytter hjertene når Teodor treffer en lavaelv.';}
  ...
  // Bakre kraterkant: sitter på platået, jagged og opplyst av lavaen nedenfra. Platåhjørnene gjemmes under kanten.
  const backHalf=width*.07,backTeeth=6,backTop=[];
  for(let g=0;g<=backTeeth;g++){backTop.push([peak.x-backHalf+backHalf*2*g/backTeeth,backY+(seeded(i,salt+100+g)-.5)*6]);}
  const backGrad=ctx.createLinearGradient(0,backY+7,0,backY-6);
  backGrad.addColorStop(0,'#241210');backGrad.addColorStop(1,'#c25a2a');
  ctx.fillStyle=backGrad;ctx.beginPath();ctx.moveTo(backTop[0][0],backTop[0][1]);
  for(let g=1;g<backTop.length;g++)ctx.lineTo(backTop[g][0],backTop[g][1]);
  for(let g=backTop.length-1;g>=0;g--)ctx.lineTo(backTop[g][0],backTop[g][1]+7);
  ctx.closePath();ctx.fill();
  ctx.save();ctx.shadowColor='#ff7a2a';ctx.shadowBlur=14;ctx.strokeStyle='rgba(255,175,85,.9)';ctx.lineWidth=2;
  ctx.beginPath();ctx.moveTo(backTop[0][0],backTop[0][1]);for(let g=1;g<backTop.length;g++)ctx.lineTo(backTop[g][0],backTop[g][1]);ctx.stroke();ctx.restore();
  ...
  const gapX=peak.x+(seeded(i,salt+130)-.5)*craterW*.7,trunkTopW=5.5+seeded(i,salt+131)*2,trunkPts=[[gapX,platY+2]];
  let trunkX=gapX;const trunkSegs=6;
  for(let g=1;g<=trunkSegs;g++){const f=g/trunkSegs;trunkX+=(seeded(i,salt+140+g)-.5)*width*.05*(.3+.7*f);trunkPts.push([trunkX,platY+2+(baseY-platY-2)*f]);}
  ...
  ctx.restore();
  ctx.save();ctx.shadowColor='#ff8a2a';ctx.shadowBlur=16;ctx.fillStyle='#ffe9a8';ctx.beginPath();ctx.arc(gapX,platY+3,3.5,0,7);ctx.fill();ctx.restore();
  // Pulserende utbruddsglød over krateret.
  const eruptionPulse=.6+.4*Math.sin(state.t*3.1+i),eruptionGlow=ctx.createRadialGradient(peak.x,platY-10,2,peak.x,platY-10,craterW*.9);
  eruptionGlow.addColorStop(0,`rgba(255,220,130,${.5*eruptionPulse})`);eruptionGlow.addColorStop(1,'rgba(255,110,30,0)');ctx.fillStyle=eruptionGlow;ctx.beginPath();ctx.arc(peak.x,platY-10,craterW*.9,0,7);ctx.fill();
  ...
  function deciduousTree(x, base, s, alpha) { ctx.save(); ctx.globalAlpha=alpha; ctx.translate(x,base); ctx.fillStyle='#2b1a14';ctx.fillRect(-s*.055,-s*.76,s*.11,s*.76);ctx.fillStyle='#4a2a20';for(const [dx,dy,r] of [[0,-.94,.3],[-.22,-.78,.27],[.24,-.76,.28],[-.05,-.59,.31]]){ctx.beginPath();ctx.arc(dx*s,dy*s,r*s,0,7);ctx.fill()}ctx.fillStyle='#7a3a22';ctx.beginPath();ctx.arc(-s*.11,-s*.86,s*.16,0,7);ctx.fill();ctx.restore(); }
  ...
  function drawMarit(){
    ...
    ctx.fillStyle='#b07a34';ctx.beginPath();ctx.arc(22,10,9,Math.PI*.55,Math.PI*1.75);ctx.lineTo(17,21);ctx.quadraticCurveTo(11,16,15,9);ctx.fill();ctx.beginPath();ctx.moveTo(30,6);ctx.lineTo(28,1);ctx.lineTo(25,4);ctx.lineTo(22,0);ctx.lineTo(19,3);ctx.lineTo(15,1);ctx.lineTo(13,5);ctx.lineTo(17,6);ctx.closePath();ctx.fill();
